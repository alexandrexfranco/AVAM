...
import { ArrowLeft, Printer, ExternalLink, AlertTriangle } from 'lucide-react';
      ...
      <div className="flex justify-center bg-gray-100 py-4 overflow-x-auto print:bg-white print:py-0 print:overflow-visible">
        ...
        <div className="border-t-2 border-black border-solid pt-1 mt-1">
           <div className="flex items-center justify-between text-[11px] font-bold uppercase mb-1.5">
             ...
             {/* Performance Summary for Report - Weighted Calculation */}
             {(() => {
               const ratings = Object.values(evaluation.ratings);
               const totalCount = ratings.length || 1;
               const countBom = ratings.filter(r => r === 'Bom').length;
               const countMedio = ratings.filter(r => r === 'Médio').length;
               const countRuim = ratings.filter(r => r === 'Ruim').length;
               
               // Pesos: Ruim e Médio agora têm um impacto muito maior no cálculo (Ponderado)
               // Ruim (peso 6), Médio (peso 3), Bom (peso 1)
               const wBom = 1;
               const wMedio = 3;
               const wRuim = 6;
               
               const totalWeight = (countBom * wBom) + (countMedio * wMedio) + (countRuim * wRuim) || 1;
               
               // Cálculo de aproveitamento (score)
               // Bom = 100%, Médio = 40%, Ruim = 0%
               const score = Math.round(((countBom * 100) + (countMedio * 40) + (countRuim * 0)) / totalCount);
               
               return (
                 <div className="flex items-center space-x-4">
                    <div className="flex space-x-3 text-[8px] font-bold text-gray-600">
                       <div>BOM: {Math.round((countBom * wBom / totalWeight) * 100)}%</div>
                       <div>MÉDIO: {Math.round((countMedio * wMedio / totalWeight) * 100)}%</div>
                       <div>RUIM: {Math.round((countRuim * wRuim / totalWeight) * 100)}%</div>
                    </div>
                    <div className="bg-slate-100 px-2 py-0.5 rounded border border-slate-200 text-[9px] font-black">
                       APROVEITAMENTO: {score}%
                    </div>
                    {evaluation.speedLimitExceededCount > 0 && (
                      <div className="bg-red-50 px-2 py-0.5 rounded border border-red-200 text-[9px] font-bold text-red-700 flex items-center">
                         <AlertTriangle className="h-2.5 w-2.5 mr-1" />
                         EXCESSOS DE VELOCIDADE: {evaluation.speedLimitExceededCount}
                      </div>
                    )}
                 </div>
               );
